            ...
            <div class="col-md-8 offset-md-2">
                ...
                <form action="{{ route('students.update', $student->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        @php
                            $phoneNumbers = json_decode($student->phone_numbers, true);
                        ...
                        <textarea name="phone_numbers" class="form-control" placeholder="Enter phone numbers separated by commas"
                            oninput="this.value=this.value.replace(/[^0-9,]/g,'');">{{ old('phone_numbers', implode(',', $phoneNumbers)) }}</textarea>
                        @error('phone_numbers')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>                    
                        ...
                        <select name="role_id" id="role_id" class="form-control">
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}"
                                    {{ old('role_id', $student->role_id) == $role->id ? 'selected' : '' }}>
                                    {{ $role->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <!-- Submit Button -->
                    <button type="submit" class="btn btn-primary mt-3" style="margin-right: 10px;">Update</button>
